...
export default {
  ...
  data() {
    return {
      ...
      validations: {
        ...
        numberTenDigits: () => this.updatedContact.number.length == 10,
      },
      numberRegex: /[0-9\b]/,
    }
  },
  ...
  methods: {
    showEditContactForm() {
      this.$emit('showEditContactForm');
    },
    hideEditContactForm() {
      this.$emit('hideEditContactForm');
    },
    formatNumber() {
      this.updatedContact.number = this.updatedContact.number.slice(0, 3) + "-" + this.updatedContact.number.slice(3, 6) + "-" + this.updatedContact.number.slice(6);
    },
    submit() {
      this.formatNumber();
...
<template>
  <div class="container">
    ...
    <div class="input-field">
      <input type="text" placeholder="First Name" v-model="updatedContact.first_name" />
    </div>
    <div class="input-field">
      <input type="text" placeholder="Last Name" v-model="updatedContact.last_name" />
    </div>
    <div class="input-field">
      <input type="text" ref="numberInput" maxlength="10" placeholder="Phone Number (digits only, no dashes)" v-model="updatedContact.number" />
    </div>
    <div class="input-field">
      <label for="personal_or_work">Personal or Work?  </label>
      <select id="personal_or_work" name="personal_or_work" v-model="updatedContact.personal_or_work">
        <option value="personal">Personal</option>
        <option value="work">Work</option>
      </select>
    </div>
    <div class="input-field">
      <textarea placeholder="Notes" rows="4" v-model="updatedContact.notes" />
    </div>
    <div class="btn-row input-field">
      <div class="btn cancel" @click="hideEditContactForm">Cancel</div>
